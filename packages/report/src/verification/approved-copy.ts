...
// This is an intentionally independent approval registry. Keep it separate from the fallback
// writer's source templates so a writer refactor cannot make verifier approval tautological.
const APPROVED_SECTION_COPY: Readonly<Record<ReportSectionKey, readonly string[]>> = Object.freeze({
  actions: [
...
/*
 * Independent compiled approval data.  This intentionally duplicates the
 * reviewed copy contract rather than importing the writer's generator: a
 * writer change cannot silently approve its own new language.
 */
const APPROVED_SECTION_LENSES: Readonly<Record<ReportSectionKey, string>> = Object.freeze({
  actions:
...
const APPROVED_EDITORIAL_STAGES = [
...
export function approvedEditorialSentence(sectionKey: ReportSectionKey, index: number): string {
  const copy = APPROVED_SECTION_COPY[sectionKey];
  const base = copy[index % copy.length] ?? copy[0] ?? "";
  const stage =
    APPROVED_EDITORIAL_STAGES[Math.floor(index / copy.length) % APPROVED_EDITORIAL_STAGES.length] ??
    "";
  const lowerFirst = (value: string) => value.charAt(0).toLocaleLowerCase("en-US") + value.slice(1);
  return `${base.replace(/[.!?]$/u, "")}; ${lowerFirst(APPROVED_SECTION_LENSES[sectionKey]).replace(/[.!?]$/u, "")}, ${lowerFirst(stage)}`;
}

export function approvedEditorialTemplateId(sectionKey: ReportSectionKey, index: number): string {
